...
# backstop in case the version key is absent. On an in-process miss (e.g.
# first request after a restart) we first look for a precomputed payload in
# the backtest_payload_cache table (written by pipeline/backtest.py) before
# recomputing, so the first load after a deploy/pipeline run stays fast.
from backtest_payloads import (
...
def v2_backtest():
    ...
    try:
        ...
        # precomputed payload written by pipeline/backtest.py?
        if version is not None:
...
def v2_peer_group_outperformance():
    ...
    try:
        ...
        db_key = f'peer-group-outperformance:{min_members}'
        # precomputed payload written by pipeline/backtest.py?
        if version is not None:
            payload = read_cached_payload(cur, db_key, version)
            if payload is not None:
                conn.close()
                _backtest_cache_put(cache_key, version, payload)
                return jsonify(payload)

        payload = compute_peer_group_outperformance_payload(cur, min_members)
        if version is not None:
            try:
                write_cached_payload(cur, db_key, version, payload)
                conn.commit()
            except Exception:
                conn.rollback()
        conn.close()
...
def get_correlation_analysis():
    ...
    (correlation-weighted culture score → composite performance) and return
    slope + R² per group. Uses the same scoring as the Culture vs Performance tab.
    """
    from scipy import stats as scipy_stats
    ...
    try:
        ...
        # ── Step 1: Bulk-load culture metrics for ALL companies in one DB hit ──
        all_companies = get_companies_for_sector()  # no filter → every company with reviews
        cached_map = {}
        conn = get_db_connection()
        if conn and all_companies:
            try:
        ...
        # Fill in uncached metrics (cap at 50 to avoid timeout)
        all_metrics = {}
        uncached = 0
        for name in all_companies:
        ...
        # ── Step 2: Bulk-load performance scores for ALL companies ──
        fmp_perf_map = _load_fmp_perf_map()
        peer_stats   = performance_analyzer.get_peer_statistics()
        all_perf = {}
        for name in all_companies:
