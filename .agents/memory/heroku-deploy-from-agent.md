...
- Never run `git fetch` from the main-agent shell: the .git guard kills git mid-fetch and orphans `refs/remotes/origin/main.lock`, which makes the workspace Git panel show INDEX_LOCKED and blocks sync. Fix: delete the lock and run the fetch from a temporary workflow process (outside the guarded shell), then remove the workflow.
