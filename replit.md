...
**Active framework (since 2026-07-21 Schroders-only rebuild): Schroders v2** — 12 bipolar dimensions (−1 to +1) scored with a mined dictionary, plus sector-relative shrinkage, evidence tiers (A–D), a ridge-model Culture Factor per company, and reliability tiers (High/Medium/Low/Insufficient). Since 2026-07-26 peer buckets are assigned **per company at the finest adequate GICS level** (sub-industry → industry → sector → global) via greedy residual counting in `pick_bucket()` (`pipeline/factor_build.py`) — disjoint buckets, each ≥ `min_companies_per_bucket` (8); residual (not full-group) counts are deliberate so ranking peer sets stay adequate (see docstring). **Adequacy update (same day, per user request):** only *model-estimable* companies (performance target + ≥`min_dims_medium` Tier A/B dims) count toward the min-8 threshold (`adequate` param of `pick_bucket`, computed in `build_shrinkage()`); non-estimable companies are still assigned to qualifying buckets. Buckets are coarser (~77 named groups vs 107) but **every named bucket now fits its own model — global_fallback eliminated**; only the residual 'global' bucket (~81 companies) uses the global model. Models/weights/classification levels are per bucket; unit tests: `pipeline/test_pick_bucket.py`. Prod must re-run `python pipeline/factor_build.py model` (Heroku one-off dyno) after deploy for scores to reflect this. New API endpoints: `/api/v2/factor-scores`, `/api/v2/evidence/<company>`, `/api/v2/model-weights`, `/api/v2/overlap-diagnostics`. Pipeline: `pipeline/factor_build.py`; validation report: `pipeline_output/schroders_rebuild_validation_report.md`.
...
The Extraction Manager "Listed Companies / Asset Managers" toggle (`window._extractionGroup`, default `listed`) drives the global header GICS filter dropdown: in listed mode the synthetic "Asset Management" pseudo-sector is hidden; in asset-managers mode it is the only option. Switching the toggle resets an invalidated filter selection to All. The Culture Factor tab (factor table, evidence dropdown) follows the global GICS filter. Since 2026-07-27 the Sector Comparison tab has ONE configurable bubble chart at the top ("Peer Group Comparison", canvas `sectorCompBubble`): X/Y/size dropdowns over 8 measures merged per peer group from `/api/v2/model-weights`, `/api/v2/culture-performance-slope`, and `/api/v2/peer-group-outperformance` (size "None" = uniform bubbles; defaults X=Model R2, Y=Q1 outperformance, size=companies in model). A fixed "Current filter" label mirrors the top GICS filter. The peer-group model-fit table sits below the chart; the dimension-weights peer-group dropdown was replaced by a fixed label (`weightsPeerLabel`) that follows the top filter when a peer group is selected there, and updates when a table row is clicked. The old tercile bar chart and the two separate scatter charts were removed.

### Culture Quartile Backtest (added 2026-07-27)
`pipeline/backtest.py` (`scores` / `prices` / `all`) computes point-in-time culture factor scores at every quarter-end since 2015-12-31 using only reviews posted by each snapshot (tiers, shrinkage w=n/(n+50) toward point-in-time bucket prior), applies TODAY's model weights/peer buckets from `schroders_sector_model_weights` (documented look-ahead), requires ≥4 Tier A/B dims, and ranks into quartiles within peer buckets → `schroders_backtest_scores`. `prices` caches quarter-end adjusted closes from FMP into `backtest_quarter_prices` (refetches any ticker missing the latest completed quarter). `/api/v2/backtest` builds equal-weight quartile portfolios (Q1=strongest culture) rebalanced quarterly vs an all-companies benchmark; since 2026-07-27 it is ALWAYS GLOBAL (GICS filter params ignored). New endpoint `/api/v2/peer-group-outperformance`: per peer bucket, last 20 quarterly snapshots, per-quartile CAGR vs the bucket's own benchmark (≥3 members/quartile per snapshot, ≥8 valid quarters); feeds the Backtest tab's "Culture Quartile Outperformance by Peer Group" horizontal bar chart (quartile dropdown) and the Sector Comparison bubble measures. UI: "Backtest" main tab (Chart.js line chart, stats table, look-ahead/survivorship caveats box). **Measure/weighting dropdowns (2026-07-27):** two selects above the chart — Measure (TSR / Earnings = chained median TTM net-income growth index / P/E = median trailing P/E level, 0<pe<200 filter) and Weighting (Current = prod model weights, Walkback = walk-forward yearly window models, Average = mean of window models) — served by `/api/v2/backtest-series?measure=&weights=` reading 9 precomputed payloads (`bt-series:{measure}:{C|A|W}` in `backtest_payload_cache`) built by `pipeline/backtest_dashboard.py payloads` (needs `/tmp/bv_pit_states.pkl` from `pipeline/backtest_variants.py pit`). Stats table shows Q1–Q4 relative to the all-companies average (row hidden; spread row absolute); P/E mode shows latest/average P/E levels instead. Prod refresh: run `python pipeline/backtest.py all` on a Heroku one-off dyno after each `factor_build.py model` run. **Payload precompute (2026-07-27):** shared compute logic lives in `backtest_payloads.py`; `pipeline/backtest.py` precomputes the `/api/v2/backtest` and `/api/v2/peer-group-outperformance` (min_members=3) payloads into the `backtest_payload_cache` table BEFORE bumping `backtest_data_version`, so the first request after a restart/deploy reads a ready-made JSON blob (endpoints also write-back on a compute miss, e.g. non-default min_members).
