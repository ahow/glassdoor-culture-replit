...
**Active framework (since 2026-07-21 Schroders-only rebuild): Schroders v2** — 12 bipolar dimensions (−1 to +1) scored with a mined dictionary, plus sector-relative shrinkage, evidence tiers (A–D), a ridge-model Culture Factor per company, and reliability tiers (High/Medium/Low/Insufficient). Since 2026-07-26 peer buckets are assigned **per company at the finest adequate GICS level** (sub-industry → industry → sector → global) via greedy residual counting in `pick_bucket()` (`pipeline/factor_build.py`) — disjoint buckets, each ≥ `min_companies_per_bucket` (8); residual (not full-group) counts are deliberate so ranking peer sets stay adequate (see docstring). Models/weights/classification levels are per bucket; unit tests: `pipeline/test_pick_bucket.py`. Prod must re-run `python pipeline/factor_build.py model` (Heroku one-off dyno) after deploy for scores to reflect this. New API endpoints: `/api/v2/factor-scores`, `/api/v2/evidence/<company>`, `/api/v2/model-weights`, `/api/v2/overlap-diagnostics`. Pipeline: `pipeline/factor_build.py`; validation report: `pipeline_output/schroders_rebuild_validation_report.md`.
