...
The Extraction Manager "Listed Companies / Asset Managers" toggle (`window._extractionGroup`, default `listed`) drives the global header GICS filter dropdown: in listed mode the synthetic "Asset Management" pseudo-sector is hidden; in asset-managers mode it is the only option. Switching the toggle resets an invalidated filter selection to All. The Culture Factor tab (factor table, evidence dropdown) follows the global GICS filter, and its "Model Fit & Dimension Weights" section shows per-peer-group ridge R² (in-sample + cross-validated) and dimension weights from `/api/v2/model-weights`. Model levels are shown with friendly labels ("Sector-specific" / "Global model*") with footnotes explaining the global fallback; the model-fit table also follows the listed/asset-managers toggle and includes a bubble scatter (x=CV R², y=in-sample R², bubble∝√companies). A "Culture vs Subsequent Share-Price Performance" bar chart (`/api/v2/culture-performance-groups`) shows avg annualized 1/3/5-yr returns by culture-factor tercile within the current filter; tickers come from `fmp_performance_metrics` (names match Glassdoor names — `fmp_ticker_map` uses legal names and does NOT match).
