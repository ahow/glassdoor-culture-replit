...
The Extraction Manager "Listed Companies / Asset Managers" toggle (`window._extractionGroup`, default `listed`) drives the global header GICS filter dropdown: in listed mode the synthetic "Asset Management" pseudo-sector is hidden; in asset-managers mode it is the only option. Switching the toggle resets an invalidated filter selection to All. The Culture Factor tab (factor table, evidence dropdown) follows the global GICS filter. Since 2026-07-26 the "Model Fit & Dimension Weights" section lives in its own top-level "Sector Comparison" tab (`sectorComparisonTab`, loads via `loadModelFit()`); per user request it shows only **in-sample** R² (cross-validated R² removed from table and chart — user wants the strongest relationship emphasized), rows sorted by R² desc, and the bubble scatter (x=companies in model, y=in-sample R², bubble∝√companies) plots group-specific models only (global-fallback groups all share the one global model — was rendering as a stack of identical bubbles at R²≈0.04). Model levels use friendly labels ("Sector-specific" / "Global model*") with footnotes explaining the global fallback; the model-fit table follows the listed/asset-managers toggle. A second bubble chart "Culture Payoff vs Strength of Relationship" (`/api/v2/culture-performance-slope`) plots per-peer-group OLS of the composite performance target (recomputed in app.py, same definition as pipeline `perf_targets()`) on `schroders_factor_sector_z`: y=slope (payoff per 1 SD culture), x=R² of that same regression, bubble∝√n, min 5 companies per group; also follows the toggle. A "Culture vs Subsequent Share-Price Performance" bar chart (`/api/v2/culture-performance-groups`) shows avg annualized 1/3/5-yr returns by culture-factor tercile within the current filter; tickers come from `fmp_performance_metrics` (names match Glassdoor names — `fmp_ticker_map` uses legal names and does NOT match).

### Culture Quartile Backtest (added 2026-07-27)
`pipeline/backtest.py` (`scores` / `prices` / `all`) computes point-in-time culture factor scores at every quarter-end since 2015-12-31 using only reviews posted by each snapshot (tiers, shrinkage w=n/(n+50) toward point-in-time bucket prior), applies TODAY's model weights/peer buckets from `schroders_sector_model_weights` (documented look-ahead), requires ≥4 Tier A/B dims, and ranks into quartiles within peer buckets → `schroders_backtest_scores`. `prices` caches quarter-end adjusted closes from FMP into `backtest_quarter_prices` (refetches any ticker missing the latest completed quarter). `/api/v2/backtest` builds equal-weight quartile portfolios (Q1=strongest culture) rebalanced quarterly vs an all-companies benchmark, honours the GICS filter, needs ≥3 members per quartile to start the series. UI: "Backtest" main tab (Chart.js line chart, stats table, look-ahead/survivorship caveats box). Prod refresh: run `python pipeline/backtest.py all` on a Heroku one-off dyno after each `factor_build.py model` run.
