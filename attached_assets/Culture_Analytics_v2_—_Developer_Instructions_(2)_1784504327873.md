...
# Note: intra-word prefixes like 'un-', 'dis-', 'anti-' are NOT included here.
# They aren't separate tokens ("unethical" is one token, not "un" + "ethical"),
# so a whole-token membership check cannot detect them. Word-boundary matching
# in 1.1 already prevents "ethical" firing inside "unethical", which is the
# main risk. If we later want to catch prefixed antonyms, they should live in
# the negative-pole dictionaries directly ("unethical", "dishonest", etc.).
...
def generate_negation_variants(positive_terms: dict[str, float]) -> dict[str, float]:
    """Generate antonyms only. Do NOT generate 'no X', 'poor X', 'lack of X' etc. —
    those phrases are handled by the negation-scope logic in Phase 1.2, which flips
    positive matches to negative when a negation marker precedes them. Adding them
    to the negative-pole dictionary as separate entries would double-count.

    This function generates only genuine antonyms — words with independent negative
    meaning that would NOT be captured by the negation handler."""
    variants = {}

    # WordNet antonyms for single-word terms
            ...
            for syn in wordnet.synsets(term):
                for lemma in syn.lemmas():
                    for ant in lemma.antonyms():
                        variant = ant.name().replace('_', ' ')
                        variants[variant] = weight * 0.8

    return variants
```

**Important — negation and Method C division of labour.**
- **Negation handling (Phase 1.2)** covers all "prefix + positive-term" patterns: "no innovation", "poor work-life balance", "lacks integrity", etc. These do NOT need to appear in the negative-pole dictionary.
- **Method C (WordNet antonyms)** covers genuine antonyms: "unethical" as opposite of "ethical", "chaotic" as opposite of "stable", "corrupt" as opposite of "honest". These DO appear in the negative-pole dictionary because they wouldn't otherwise be captured.
- **Sanity check to prevent double counting:** before adding a generated negative-pole term, verify that the review "poor innovation" is not already scored negative-innovation by the negation handler. If it is, do not add "poor innovation" as a separate negative-pole term.

Filter Method C outputs back through Method A (embedding similarity to negative-pole seeds) to drop artifacts — many WordNet antonyms are not phrases people actually write in reviews.
...
**Check 4 — Known-company sanity test.** Pick 6 companies from the actual universe (i.e. with 500+ reviews in the current corpus) that have well-established public cultural reputations. Score them under the new dictionary and verify the dominant pole for each dimension matches the reputation.

Recommended candidates from the actual review universe:

| Company | Reviews | Expected cultural fingerprint |
|---|---|---|
| Costco Wholesale | 19,149 | b09 supportive, b02 thrifty, b06 externally-focused (customer), b10 stable |
| Goldman Sachs | 23,313 | b03 hierarchical, b05 individual, b07 risk-taking, b10 stable, b08 meritocratic |
| Salesforce | 24,474 | b01 long-term, b02 growth-focused, b12 diverse, b06 externally-focused |
| Wells Fargo | 58,315 | b11 compliance issues (2016-onwards scandals visible), b03 hierarchical, b10 chaotic |
| McDonald's | 27,750 | b04 rules-driven, b03 hierarchical, b10 stable, b02 cost-focused |
| Microsoft | 14,500 | b01 long-term, b09 supportive (post-Nadella), b03 relatively egalitarian, b12 diverse |

These are chosen for well-documented reputations in the public record (news coverage, ex-employee accounts, regulatory filings for Wells Fargo, transformation narratives for Microsoft) rather than for being "good" or "bad" companies. The point is that the dictionary should score them recognisably.

This is not manual keyword labelling — it's a sanity check on the aggregate output for known cases. If Goldman scores as supportive-and-egalitarian, or Wells Fargo scores as high-integrity, the dictionary has clearly failed and needs iteration on seeds or thresholds.
...
def enriched_r2(y: np.ndarray, y_pred: np.ndarray, n_predictors: int) -> dict:
    ...
    n_predictors: number of dimensions used to build the composite.
                  Under v2: 12 for Schroders (b01-b12), 9 for MIT, 6 for Hofstede,
                  27 for the combined framework."""
    n = len(y)
...
## Phase 6 — Database and dashboard migration

**Goal.** Migrate the storage and UI layer from the current 18-dimension Schroders framework to the v2 12-dimension bipolar framework, running both in parallel during the transition.

### 6.1 Database schema changes

**Add v2 score columns.** In `company_culture_scores` and `review_culture_scores`, add:
- `schroders_v2_b01_score` through `schroders_v2_b12_score` (FLOAT, nullable)
- `schroders_v2_b01_evidence` through `schroders_v2_b12_evidence` (INT, defaults 0)
- `schroders_v2_composite_equalwt` (FLOAT, nullable)
- `schroders_v2_composite_corrwt` (FLOAT, nullable)

Keep v1 columns (`schroders_d01_score` through `d18_score`) alive during the transition. Do not drop them until v2 has been reviewed and signed off. This lets both frameworks be compared side-by-side.

**Update the `dictionary_version` and `scoring_engine_version` columns** (added in Phase 1.3) so v1 rows are tagged `2026-04-20` / `v1-substring` and v2 rows are tagged `2026-08-01-v2` / `v2.0-regex-negation`.

**Add a `schroders_framework_active` column to a config table** so the dashboard can be switched between v1 and v2 without a code deploy.

### 6.2 Dashboard changes

- New labels and tooltips for each of the 12 bipoles. The label should show both poles (e.g. "Short-term ↔ Long-term" not "Long-term Orientation") so users understand the score is bipolar.
- New chart layouts: replace the current 18-dimension radar/heatmap with 12-dimension equivalents. Colour scale should be diverging (red ↔ blue or similar), centred on zero, to visually reinforce bipolarity.
- Comparison view: when v1 and v2 are both live, provide a toggle so a user can see the same company under both frameworks.
- Sub-industry R² matrix: rebuild with v2 dimensions. Expose the enriched-R² metrics (raw + adjusted + bootstrap CI) alongside the primary number.
- Score confidence display: companies with fewer than 20 total reviews should have their scores displayed greyed-out with a tooltip explaining low confidence. Companies with fewer than 5 reviews should have scores hidden entirely.

### 6.3 Public-facing changes

- Update `METHODOLOGY.md` (see Meta-deliverable below)
- Update the About/Help pages
- If there is a published API surface, version it (`/api/v2/culture-scores/...`) so any external consumers can migrate on their own schedule

**Estimated effort for Phase 6:** 3–5 days including migration scripts, dashboard rebuild, and a review pass. This assumes the v2 scoring is stable and no schema iteration is required.

---

## Compute environment and dependencies

**Important:** the pipeline steps in Phases 2 and 3 must NOT run inside the web application process on Heroku.

Past experience with this project has shown that long-running jobs on the web dyno get killed by Heroku's request timeout or dyno restart cycles. The following operations must run offline (locally on a developer machine or as a one-off Heroku worker dyno, ideally with a Performance-M or better tier):

**Offline pipeline components:**
- Phrase mining with spaCy over the 3.4M review corpus (Phase 2.2 Method B) — expect ~2–6 hours depending on hardware
- Embedding ~100k candidate phrases via sentence-transformers (Phase 2.2 Method A) — expect ~30–90 minutes on CPU, 5–15 minutes on GPU
- Re-scoring the full universe under new dictionaries (Phase 3.3) — expect ~4–12 hours depending on parallelisation

**Pipeline-only dependencies (do NOT add to the web app's `requirements.txt`):**
- `sentence-transformers` (~1 GB with default model download)
- `spacy` and `en_core_web_sm` (~50 MB)
- `nltk` and WordNet corpus (~20 MB)
- `scikit-learn` (already present, keep)

Maintain a separate `pipeline_requirements.txt` for these. The pipeline outputs (dictionaries as Python files, scored review data as SQL inserts or CSVs for bulk-load) are then imported by the web app.

**Suggested pipeline structure:**
```
/pipeline
  build_dictionaries.py       # Phase 2 — one-shot, runs offline
  score_corpus.py              # Phase 3.3 — one-shot per dictionary version, runs offline
  validate_dictionaries.py     # Phase 2.5 automated checks
  validate_framework.py        # Phase 3.4 statistical tests
  requirements.txt             # pipeline-only deps
/app
  culture_scoring.py           # scoring functions used by both pipeline and app (thin, no ML deps)
  schroders_v2_keywords.py     # dictionary file — output of pipeline
  ...
  requirements.txt             # web app deps — must not include ML libraries
```

The web app should import scoring logic and dictionaries but never trigger corpus-scale processing.
...
- Phase 6 (database and dashboard migration): 3–5 days

Total: ~8–10 weeks of focused work. This is shorter than the hand-labelled variant (~11 weeks) because Phase 2 is fully automated.
